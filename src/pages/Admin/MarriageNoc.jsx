...
import {
  ...
  generatePDF,
  viewPDF
} from '../../api/marriageNocService';
import MarriageNocForm from '../../components/certificate/MarriageNocForm';
import MarriageNocTable from '../../components/certificate/MarriageNocTable';
import MarriageNocViewModal from '../../components/certificate/MarriageNocViewModal';
...
const MarriageNoc = () => {
  ...
  const [editingNoc, setEditingNoc] = useState(null);
  const [viewModal, setViewModal] = useState({ isOpen: false, noc: null });

  useEffect(() => {
    fetchNocs(page, searchTerm);
  }, [page, searchTerm]);

  const fetchNocs = async (currentPage = page, currentSearch = searchTerm) => {
    try {
      setLoading(true);
      const params = { page: currentPage, limit: 10 };
      if (currentSearch) params.search = currentSearch;
      const data = await getAllNocs(params);
      setNocs(data.nocs);
      setTotal(data.total);
      setPages(data.pages);
    } catch (error) {
      toast.error('Failed to fetch NOC records');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreate = async (data) => {
    try {
      await createNoc(data);
      toast.success(t('noc.nocCreated'));
      setShowForm(false);
      fetchNocs();
    } catch (error) {
  ...
  const handleUpdate = async (id, data) => {
    try {
      await updateNoc(id, data);
      toast.success(t('noc.nocUpdated'));
      setEditingNoc(null);
      setShowForm(false);
      fetchNocs();
    } catch (error) {
      toast.error('Failed to update NOC');
      console.error(error);
    }
  };
  ...
  const handleView = async (id) => {
    try {
      const noc = await getNocById(id);
      setViewModal({ isOpen: true, noc });
    } catch (error) {
      toast.error('Failed to fetch NOC details');
      console.error(error);
    }
  };
  ...
  const handleEdit = async (id) => {
    try {
      const noc = await getNocById(id);
      // Normalise date fields for the <input type="date"> defaultValues.
      const toInputDate = (d) => (d ? new Date(d).toISOString().slice(0, 10) : '');
      setEditingNoc({
        ...noc,
        date: toInputDate(noc.date),
        nikkah_date: toInputDate(noc.nikkah_date)
      });
      setShowForm(true);
    } catch (error) {
      toast.error('Failed to fetch NOC details');
  ...
  const handleDownload = async (id) => {
    try {
      ...
      if (!noc.pdf_url) {
        ...
        if (response.pdf_url) {
          noc.pdf_url = response.pdf_url;
        } else {
          toast.error('PDF generation failed');
          return;
        }
      }
      window.open(resolveCertUrl(noc.pdf_url), '_blank');
  ...
  const handleFormSubmit = (data) => {
    if (editingNoc) {
      handleUpdate(editingNoc._id, data);
    } else {
      handleCreate(data);
    }
  };
    ...
    <div>
      ...
      <div className="bg-white dark:bg-[#1e1f25] rounded-2xl p-4 border border-gray-100 dark:border-gray-800 mb-6">
        <div className="flex gap-3">
          ...
          <button
            onClick={() => fetchNocs(page, searchTerm)}
            disabled={loading}
      ...
      <AnimatePresence>
        {viewModal.isOpen && (
          <MarriageNocViewModal
            noc={viewModal.noc}
            onClose={() => setViewModal({ isOpen: false, noc: null })}
            onDownload={handleDownload}
            onPrint={handlePrint}
          />
        )}
