...
export default function HouseDetailedView() {
    ...
    useEffect(() => { fetchData(); }, [id, currentPage, itemsPerPage]);

    if (loading && !house) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="w-8 h-8 border-4 border-[#0B65F6] border-t-transparent rounded-full animate-spin" />
            </div>
        );
    }
    ...
    if (!house) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-gray-500 text-center">
                ...
                <h2 className="text-xl font-bold mb-2">{t('family.houseDetail.notFound')}</h2>
                <button onClick={() => navigate('/family/house/register')} className="text-[#0B65F6] hover:underline mt-4">
                    {t('family.houseDetail.backToList')}
                </button>
            </div>
        );
    }
    ...
    const memberColumns = [
        {
            header: t('family.houseDetail.colName'),
            cell: (row) => (
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                            {row.full_name}
                            {row.is_family_head && (
                                <span className="px-1.5 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 text-[9px] rounded-md uppercase font-bold tracking-wider">
                                    {t('family.head')}
                                </span>
                            )}
                        </div>
                        <div className="text-[10px] text-gray-500 uppercase tracking-wider">
                            {row.relation_to_head || t('family.houseDetail.memberFallback')}
                        </div>
                    </div>
                </div>
            )
        ...
        {
            header: t('family.houseDetail.colDob'),
            cell: (row) => {
                if (!row.dob) return '-';
                ...
                return (
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-gray-500">{age} {t('family.houseDetail.yearsOld')}</span>
                    </div>
                );
            }
        },
        {
            header: t('family.houseDetail.colDetails'),
            cell: (row) => (
        ...
        {
            header: t('family.houseDetail.colStatus'),
    ...
    return (
        <div className="w-full space-y-6">
            ...
            <div className="flex flex-col gap-6">
                ...
                >
                    <div className="flex md:flex-row flex-col md:items-center justify-between mb-6 pb-4 border-b border-gray-100 dark:border-gray-800 gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0 self-start md:self-auto ml-[60px] md:ml-0">
                            <span className={`inline-block px-3 py-1 rounded-lg font-bold ${
                                house.economic_status === 'Poor' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                            ...
                            {house.zakat_eligible && (
                                <span className="inline-block px-3 py-1 rounded-lg font-bold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                                    {t('family.houseDetail.zakatEligible')}
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">{t('family.family')}</p>
                                <p className="text-gray-900 dark:text-gray-200 font-medium">{house.family_id?.family_name || '-'}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">{t('family.houseDetail.contact')}</p>
                                <p className="text-gray-900 dark:text-gray-200 font-medium">{house.primary_contact || '-'}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">{t('family.houseDetail.address')}</p>
                                <p className="text-gray-900 dark:text-gray-200 font-medium leading-relaxed">{house.address || '-'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {house.notes && (
                        <div className="bg-orange-50 dark:bg-orange-500/10 rounded-xl p-3 border border-orange-100 dark:border-orange-500/20 mt-6">
                            <p className="text-[10px] text-orange-600 dark:text-orange-400 uppercase tracking-wider font-bold mb-1">{t('family.houseDetail.notes')}</p>
                ...
                >
                    <DataTable
                        title={t('family.houseDetail.membersTitle')}
                        subtitle={`${t('family.houseDetail.membersSubtitle')} ${house.house_code}`}
