...
import {
  BookOpen, Plus, X, Edit2, Trash2, Upload,
...
import {
  listSurahs, createSurah, updateSurah, deleteSurah, getPdfUrl
...
export default function SurahLibrary() {
  const { t } = useTranslation();
  const [data,      setData]      = useState({ surahs: [], total: 0, page: 1, pages: 1 });
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    defaultValues: { title: '', arabic_title: '', description: '', display_order: 0 }
  });

  const fetch = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await listSurahs({ page, limit: 20, search });
      setData(res);
  ...
  const openAdd = () => {
    ...
    reset({ title: '', arabic_title: '', description: '', display_order: 0 });
  ...
  const openEdit = (item) => {
    ...
    reset({
      title: item.title, arabic_title: item.arabic_title || '',
      description: item.description || '', display_order: item.display_order || 0,
    });
    if (fileRef.current) fileRef.current.value = '';
  };
  ...
  const onSubmit = async (values) => {
    ...
    try {
      if (modal.item) {
        await updateSurah(modal.item._id, fd);
        toast.success('Surah updated');
      } else {
        await createSurah(fd);
  ...
  const handleDelete = async (item) => {
    const result = await Swal.fire({
      title: 'Delete Surah?', text: `"${item.title}" will be removed.`,
    ...
    <div className="w-full">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <BookOpen size={24} className="text-[#0B65F6]" />
            {t('islamicLibrary.surah.title', { defaultValue: 'Surah Library' })}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
            {t('islamicLibrary.surah.desc', { defaultValue: 'Manage Islamic Surah PDFs for the public portal' })}
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-white dark:bg-[#1e1f25] rounded-2xl border border-gray-100 dark:border-gray-800 mb-6">
        <div className="p-4 flex gap-3">
          ...
          {search && (
            <button onClick={() => setSearch('')} className="px-4 py-2 bg-red-500 text-white rounded-xl text-sm hover:bg-red-600">
              <X size={16} />
            </button>
          )}
        </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800">
                      ...
                      <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase">Arabic</th>
                  ...
                  <tbody>
                    {data.surahs.length === 0 ? (
                      ...
                      <tr key={item._id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50/50 dark:hover:bg-gray-800/20">
                        ...
                        <td className="py-3 px-4 text-gray-500 font-arabic text-base">{item.arabic_title || '—'}</td>
                  ...
                  <div key={item._id} className="bg-gray-50 dark:bg-[#252731] rounded-xl p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {item.arabic_title && <p className="text-gray-500 text-sm font-arabic">{item.arabic_title}</p>}
                      </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="p-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                  {modal.item ? 'Edit Surah' : 'Add Surah'}
                </h2>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Title (English) *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Arabic Title</label>
                  <input {...register('arabic_title')} dir="rtl" placeholder="سورة يس"
                    className="w-full px-4 py-2 bg-gray-50 dark:bg-[#252731] border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0B65F6] text-right font-arabic" />
                </div>
